...
class prediction:
    def __init__(self):
        pass
    def RBsingle(self, input, dataframe):
        final = []
        value= []
        chan_b = []
        chan_r = []
        e = 1e-11
        threshold = np.min(dataframe['Blue channel'])
        for i in input:
            R, _, B = cv2.split(i)
            B = B + e
    ...
    def total_prediction(self, image_path, mask_path, crop_size=570, properties=None, kmeans=None, GMM=None, df=None):
        ...
        final, value, RB = self.RBsingle(images, dataframe=df)
        ...
        x = test.drop(columns=['Blue channel', 'intensity', 'Red channel'])
    ...
    def weighted_prediction(self,weight:None,predicted_result:list, cloud_percent:float,sky_status=None):
        ...
        if (sky_status == 'Mostly Cloudy') or (sky_status == 'Overcast'):
            risk_factor =  weight[1] * predicted_result[1] + weight[2] * cloud_percent + weight[3] * 1.2
        else:
            risk_factor =  weight[1] * predicted_result[1] + weight[2] * cloud_percent + weight[3]
        return risk_factor
