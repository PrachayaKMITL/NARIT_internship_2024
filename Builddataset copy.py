...
def copy_categorical_day(image_directory, output_directory, mask_directory, classes:dict, mode):
    ...
    mask = preprocessData().crop_center(mask, crop_size=1300)

    # Load and preprocess images
    images, name = preprocessData().load_images_and_preprocess(path=image_directory, mask=mask, apply_crop_sun=False, size=1300)
...
def process_image_folders(main_directory):
    ...
    for subdir, _, _ in os.walk(main_directory):
        copy_categorical_day(subdir, r'C:\Users\ASUS\Documents\NARIT_internship_data\Dataset\Image_data_Australia\Image_data_Day', 
